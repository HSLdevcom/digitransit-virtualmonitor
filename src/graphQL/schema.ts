...
import SConfiguration, {
  defaultValue as defaultConfiguration,
  SConfigurationInput,
} from './SConfiguration';
...
let current: IData = (() => {
  ...
  const displays = configurations
    .map(c => c.displays)
    .reduce((acc: IDisplay[], cur) => [...acc, ...cur], []);
  const views = displays
    .map(d => d.viewCarousel)
...
const configurationsResolve = (
  _: any,
  { ids, name }: { ids?: ReadonlyArray<string>; name?: string },
): ReadonlyArray<IConfiguration> => {
  if (ids) {
    return current.configurations.filter(c => c.id && ids.includes(c.id));
  } else if (name) {
    const foundConfiguration = current.configurations.find(
      c => c.name === name,
    );
    return foundConfiguration ? [foundConfiguration] : [];
  } /* if (!ids && !name) Implied */ else {
  ...
  mutation: new GraphQLObjectType({
    fields: () => ({
      ...
      modifyConfiguration: {
        ...
        description:
          'Modify or create a Configuration, returns the modified/created Configuration',
        resolve: (
          _,
          {
            configuration,
          }: { configuration: IConfiguration; configurationId: string },
        ) => {
          ...
          current = {
            ...
            configurations: [
              ...current.configurations.filter(
                conf => conf.name !== newConfiguration.name,
              ),
              newConfiguration,
            ],
          };
      ...
      saveConfiguration: {
        ...
        description:
          'Add a new Configuration, returns the newly created Configuration',
        resolve: (
          _,
          {
            configuration,
          ...
          current = {
            ...current,
            configurations: [...current.configurations, newConfiguration],
          };
          return newConfiguration;
        },
        type: SConfiguration,
    ...
    fields: {
      configurations: {
        ...
        type: new GraphQLNonNull(
          new GraphQLList(new GraphQLNonNull(SConfiguration)),
        ),
      },
      node: {
        args: {
        ...
        resolve: (_, { id }: { id: string; name: string }) => {
          if (id) {
            if (current.configurations.find(c => c.id === id)) {
              return {
                ...current.configurations.find(c => c.id === id),
                __ownTypeName: 'Configuration',
              };
            }
            if (current.views.find(v => v.id === id)) {
              ...
              return {
                ...foundView,
                __ownTypeName: viewTypeMap[(foundView as IViewBase).type],
              };
            }
            if (current.displays.find(d => d.id === id)) {
              return {
                ...current.displays.find(d => d.id === id),
                __ownTypeName: 'Display',
              };
            }
          }
          return null;
