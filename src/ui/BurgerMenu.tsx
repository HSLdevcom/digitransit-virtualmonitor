...
const BurgerMenu: React.FC<Props & WithTranslation> = ({
  ...
  return (
    <Menu
      ...
      disableAutoFocus
    >
      <section id="languages" arial-label={t('')} style={{ display: 'flex' }}>
        <Link
          className="lang-select"
          onClick={() => changeLanguage(i18n, 'fi')}
          to={window.location.pathname}
          aria-label={t('changeLanguage', { language: t('languageNameFi') })}
        >
          FI
        </Link>
        <Link
          className="lang-select"
          onClick={() => changeLanguage(i18n, 'sv')}
          to={window.location.pathname}
          aria-label={t('changeLanguage', { language: t('languageNameSv') })}
        >
          SV
        </Link>
        <Link
          ...
          to={window.location.pathname}
          aria-label={t('changeLanguage', { language: t('languageNameEn') })}
        >
          EN
        </Link>
      </section>
      <section id="links" style={{ display: 'flex' }}>
        ...
        <Link
          className="link"
          to={user && user.loggedIn ? '/createStaticView' : '/createView'}
        >
          {t('createViewTitle')}
        </Link>
