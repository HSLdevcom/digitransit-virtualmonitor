...
import ConfigurationRetriever, {
  ConfigurationRetrieverResult,
} from '../ui/ConfigurationRetriever';
...
const ConfigurationDisplay = ({
  ...
  displayName,
  t,
}: IConfigurationDisplayProps & WithTranslation) => {
  return (
    <ConfigurationRetriever name={configurationName}>
      {(result: ConfigurationRetrieverResult): React.ReactNode => {
        ...
        if (!result || !result.data) {
          return (
            <div>
              {t('configurationRetrieveError')} -{' '}
              {result.error ? result.error.message : 'Unspecified error'}
            </div>
          );
        }
        ...
        if (
          ...
          return <div>{t('configurationRetrieveNotFound')}</div>;
        }
        ...
        const usedConfiguration = result.data.configurations.find(
        ...
        const usedDisplay =
          usedConfiguration.displays.find(
            display => display.name === displayName,
          ) || usedConfiguration.displays[0];
        if (!usedDisplay) {
          return null;
        }

        return <ViewCarousel viewCarousel={usedDisplay.viewCarousel} />;
      }}
