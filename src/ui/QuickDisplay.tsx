...
class QuickDisplay extends React.Component<
  IQuickDisplayProps & { virtualMonitor: typeof virtualMonitorClient },
  IQuickDisplayState
> {
  constructor(
    props: IQuickDisplayProps & { virtualMonitor: typeof virtualMonitorClient },
  ) {
    super(props);
    this.state = {};
    if (props.match.params.version && props.match.params.packedDisplay) {
      pairs[props.match.params.version]
        .unpack(decodeURIComponent(props.match.params.packedDisplay))
        .then((unpacked: IDisplay) => {
      ...
      props.virtualMonitor
        ...
        .then(
          ({
            ...
            data?: { addQuickConfiguration: IConfiguration };
            errors?: any;
          }) => {
            if (errors) {
        ...
        {({ virtualMonitor }) => (
          <>
            ...
            {this.state.displayId ? (
              ...
              >
                {(
                  result: QueryResult<{ display: IDisplay }>,
                ): React.ReactNode => {
                  if (result.loading) {
  ...
  protected insertDisplayToCache(
    virtualMonitor: typeof virtualMonitorClient,
    display: IDisplay,
  ) {
...
class DisplayEditorHistoryUpdater extends React.PureComponent<IDisplayEditorHistoryUpdaterProps> {
  ...
  public static minimizeDisplay(display: IDisplay): IDisplay {
    const removeIDAndTypename = (o: {
      id?: string;
      __typename?: string;
    }): {} => {
      const { id, __typename, ...rest } = {
        ...o,
        id: undefined,
    ...
    const recursive = (o: { [index: string]: any }): {} => {
      return Object.getOwnPropertyNames(removeIDAndTypename(o))
        ...
        .map(propName => {
          ...
          return [propName, minimizedPropType];
        })
        .reduce(
  ...
  public componentDidUpdate(prevProps: { display: IDisplay }) {
    if (
      ...
      JSON.stringify(this.props.display) !== JSON.stringify(prevProps.display)
    ) {
      const sanitized = DisplayEditorHistoryUpdater.minimizeDisplay(
        this.props.display,
      );
      const version = 'v0';
      pairs[version]
        ...
        .then((packed: any) =>
          this.props.history.push(
            `/quickDisplay/${version}/${encodeURIComponent(packed)}`,
          ),
        );
    }
  }
  ...
  public render() {
    return <DisplayEditor display={this.props.display} />;
  }
}
...
export default (props: IQuickDisplayProps) => (
  <ApolloClientsContext.Consumer>
    {({ virtualMonitor }) => (
      <QuickDisplay virtualMonitor={virtualMonitor} {...props} />
    )}
  </ApolloClientsContext.Consumer>
);
